...
function Funfacts() {
  ...
  return (
    ...
    <>
    {/* For lg screens */}
     <div className='xl:grid cs:hidden xl:grid-cols-4 gap-4 place-items-center px-10'>
        {fact.map((items)=>{
            return(
    ...
    {/* For other screeens */}
        <div className='cs:no-scrollbar cs:grid lg:hidden cs:grid-flow-col w-full max-w-[calc(100vw-2rem)] overflow-scroll gap-2 justify-start mx-auto '>
        {fact.map((items)=>{
            return(
                <div key={items.id} className='bg-white py-10 px-7 rounded-lg w-[300px]'>
                    <div className='text-3xl font-bold pb-2'> {items.title}</div>
